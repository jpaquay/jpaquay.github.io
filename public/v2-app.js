...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (slsaBuildBtn) {
    slsaBuildBtn.addEventListener('click', () => {
      ...
      if (slsaConsoleLog) {
        ...
        slsaConsoleLog.innerHTML = `🔨 <strong>[Hermetic Worker]</strong> Disabling network access. Compiling in ephemereal container with pinned toolchain...`;
      }
    });
  }
  ...
  if (slsaAttestBtn) {
    slsaAttestBtn.addEventListener('click', () => {
      ...
      if (slsaConsoleLog) {
        ...
        slsaConsoleLog.innerHTML = `🔏 <strong>[In-Toto Attestation]</strong> Generated Cosign/Sigstore cryptographically verifiable provenance payload.`;
      }
    });
  }
  ...
  if (slsaVerifyBtn) {
    slsaVerifyBtn.addEventListener('click', () => {
      ...
      if (slsaConsoleLog) {
        slsaConsoleLog.className = "vp-sim-log pulse-success";
        slsaConsoleLog.innerHTML = `🛡️ <strong>[SLSA L4 Policy Engine]</strong> Verified non-falsifiable provenance, 2-party review, and hermetic build. Deployment authorized!`;
      }
    });
  }
  ...
  if (greenSlider) {
    greenSlider.addEventListener('input', (e) => {
      ...
      if (!greenIsMigrated && greenConsoleLog) {
        if (val > 300) {
          greenConsoleLog.innerHTML = `⚠️ <strong>[High Carbon Alert]</strong> Brussels grid intensity peaked at ${val} gCO2/kWh (coal/gas generation active). Click button to reroute to green Nordic hydro!`;
        } else {
          greenConsoleLog.innerHTML = `🌱 <strong>[Green FLOPs Engine]</strong> Brussels grid carbon intensity: ${val} gCO2/kWh.`;
        }
      }
    });
  }
    ...
    greenMigrateBtn.addEventListener('click', () => {
      ...
      if (greenIsMigrated) {
        ...
        if (greenConsoleLog) {
          ...
          greenConsoleLog.innerHTML = `🌱 <strong>[Carbon Rerouting Active]</strong> Workload shifted to europe-north1 (Hamina Hydro/Wind DC). 84% reduction in carbon footprint!`;
        }
      } else {
        ...
        if (greenConsoleLog) {
          ...
          greenConsoleLog.innerHTML = `📍 <strong>[Local Grid Active]</strong> Workload running in europe-west1 (Brussels).`;
        }
      }
    });
  }
  ...
  if (aiactHighriskBtn) {
    aiactHighriskBtn.addEventListener('click', () => {
      ...
      if (aiactConsoleLog) {
        ...
        aiactConsoleLog.innerHTML = `🏛️ <strong>[EU AI Act Audit]</strong> High-Risk classification verified. Human oversight, logging, & fundamental rights assessment passed.`;
      }
    });
  }
  ...
  if (aiactGpaiBtn) {
    aiactGpaiBtn.addEventListener('click', () => {
      ...
      if (aiactConsoleLog) {
        aiactConsoleLog.className = "vp-sim-log pulse-success";
        aiactConsoleLog.innerHTML = `🤖 <strong>[GPAI Code of Practice]</strong> Automated evaluation against EU AI Office rules: Copyright compliance & red-teaming report generated.`;
      }
    });
  }
  ...
  if (aiactDoraBtn) {
    aiactDoraBtn.addEventListener('click', () => {
      ...
      if (aiactConsoleLog) {
        aiactConsoleLog.className = "vp-sim-log pulse-success";
        aiactConsoleLog.innerHTML = `🔐 <strong>[DORA & NIS2 Crucible]</strong> ICT third-party risk management verified. Continuous threat monitoring and RTO < 5m certified.`;
      }
    });
  }
